...
def check_readme_artifact_coverage() -> list[Tuple[Path, Sequence[str]]]:
    readme = Path("README.md")
    content = readme.read_text(encoding="utf-8")
    linked = {
        Path(match)
        for match in MARKDOWN_LINK_PATTERN.findall(content)
        if match.startswith("artifacts/") and match.endswith(".md")
    }
    ...
    missing = sorted(path.as_posix() for path in artifact_paths - linked)
    if missing:
        failures.append((readme, [f"public artifact markdown not indexed in README: {', '.join(missing)}"]))

    invalid_statuses: list[str] = []
    for path in sorted(linked):
        if not path.exists() or path.name == "PULL_REQUEST_TEMPLATE.md":
            continue
        text = path.read_text(encoding="utf-8")
        fm_data, _ = parse_frontmatter(text)
        status = str(fm_data.get("status", "")).strip()
        if status not in {"public", "public-draft"}:
            invalid_statuses.append(f"{path.as_posix()} -> {status or 'missing'}")
    if invalid_statuses:
        failures.append((readme, [f"README-linked artifacts must be public or public-draft: {', '.join(invalid_statuses)}"]))
